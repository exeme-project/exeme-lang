 ...
 */
__attribute__((noreturn)) void parser_error(struct Parser*				self,
	...
	printf("-%s> %s\n%s | %s\n%s", repeatChr('-', lineNumberStringLength), self->lexer->FILE_PATH,
		   lineNumberString, line->_value, repeatChr(' ', lineNumberStringLength + 3));
	...
	if (failed) {
		printf("%s^ parser received a token with an identifier it does not support yet; defaulting "
...
void parser_parseChrOrString(struct Parser* self, const struct LexerToken* lexerToken) {
	if (lexerToken->identifier == LEXERTOKENS_CHR) {
		array_append(self->parserTokens, ast_new(ASTTOKENS_CHR, AST_CHR, lexerToken,
												 string_new(lexerToken->value->_value, true)));
	} else {
		array_append(self->parserTokens, ast_new(ASTTOKENS_STRING, AST_STRING, lexerToken,
												 string_new(lexerToken->value->_value, true)));
	}
}
...
void parser_parseNumber(struct Parser* self, const struct LexerToken* lexerToken) {
	if (lexerToken->identifier == LEXERTOKENS_INTEGER) {
		array_append(self->parserTokens, ast_new(ASTTOKENS_INTEGER, AST_INTEGER, lexerToken,
												 string_new(lexerToken->value->_value, true)));
	} else {
		array_append(self->parserTokens, ast_new(ASTTOKENS_FLOAT, AST_FLOAT, lexerToken,
												 string_new(lexerToken->value->_value, true)));
	}
}
...
void parser_parseFunction(struct Parser* self, const struct LexerToken* funcKeywordLexerToken,
	...
	struct AST *function = NULL, *identifier = NULL, *openingBrackets = NULL, *arguments = NULL,
			   *argumentIdentifier = NULL, *argumentTypeSeparator = NULL, *argumentType = NULL,
			   *closingBrackets = NULL,
			   *lastToken = NULL; // arguments are placeholders, so its better to use that word
	...
	if (parsedIdentifier) { // if this is a function call
		...
		if (!parser_parse(self, false, false)) {
			lexer_error(self->lexer, P0001, "expected 1 parser token after 'func', got 0",
						funcKeywordLexerToken);
		}
	...
	while (true) {
		...
		// TODO: You know... work out if it is actually a type. And that requires types to be
		// supported... EDIT ITS NOT THAT'S FOR THE COMPILER :). YAY, ITS SO MUCH EASIER NOW!
 ...
 */
void parser_parseIdentifier(struct Parser* self, const struct LexerToken* lexerToken,
							size_t lexerTokenIndex) {
	bool pointer = false;

	if (self->lexer->tokens->length > 1) {
		if (((const struct LexerToken*)array_get(self->lexer->tokens, lexerTokenIndex - 1))
				->identifier
...
void parser_parseAssignment(struct Parser* self, const struct LexerToken* lexerToken) {
	...
	if (!parser_parse(self, false, false)) {
		lexer_error(self->lexer, P0001, "expected 1 parser token after assignment, got 0",
					lexerToken);
	}
	...
	case LEXERTOKENS_MULTIPLICATION_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_MULTIPLICATION_ASSIGNMENT, AST_MULTIPLICATION_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	...
	case LEXERTOKENS_FLOOR_DIVISION_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_FLOOR_DIVISION_ASSIGNMENT, AST_FLOOR_DIVISION_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	...
	case LEXERTOKENS_SUBTRACTION_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_SUBTRACTION_ASSIGNMENT, AST_SUBTRACTION_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	case LEXERTOKENS_BITWISE_AND_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_BITWISE_AND_ASSIGNMENT, AST_BITWISE_AND_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	...
	case LEXERTOKENS_BITWISE_XOR_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_BITWISE_XOR_ASSIGNMENT, AST_BITWISE_XOR_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	case LEXERTOKENS_BITWISE_NOT_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_BITWISE_NOT_ASSIGNMENT, AST_BITWISE_NOT_ASSIGNMENT, lexerToken,
					(const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	case LEXERTOKENS_BITWISE_LEFT_SHIFT_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_BITWISE_LEFT_SHIFT_ASSIGNMENT, AST_BITWISE_LEFT_SHIFT_ASSIGNMENT,
					lexerToken, (const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	case LEXERTOKENS_BITWISE_RIGHT_SHIFT_ASSIGNMENT:
		self->AST =
			ast_new(ASTTOKENS_BITWISE_RIGHT_SHIFT_ASSIGNMENT, AST_BITWISE_RIGHT_SHIFT_ASSIGNMENT,
					lexerToken, (const struct AST_VARIABLE*)identifier->data.AST_ASSIGNMENT, value);
		break;
	default:
...
bool parser_parse(struct Parser* self, bool freeParserTokens, bool nextLine) {
	...
	do {
		if (!lexer_lex(self->lexer, nextLine)) {
			...
			if (!nextLine
				&& self->parserTokens->length != 0) { // If not allowed to go to next line and
